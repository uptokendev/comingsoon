...
async function sendApprovalEmail(row: RecruiterRow, config: ReturnType<typeof getConfig>) {
...
async function getRowById(baseUrl: string, serviceKey: string, table: string, id: number) {
...
async function patchTable(baseUrl: string, serviceKey: string, table: string, id: number, updateBody: Record<string, unknown>) {
  const url = new URL(`${baseUrl.replace(/\/$/, '')}/rest/v1/${table}`)
  url.searchParams.set('id', `eq.${id}`)
  url.searchParams.set('select', SELECT_FIELDS)
...
async function patchRow(baseUrl: string, serviceKey: string, table: string, payload: ReviewPayload, config: ReturnType<typeof getConfig>): Promise<PatchResult> {
  ...
  try {
    await sendApprovalEmail(row, config)
...
export default async function handler(req: any, res: any) {
  ...
  try {
    if (req.method === 'GET') {
      ...
      return json(res, 200, { ok: true, counts, rows })
    }

    const result = await patchRow(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, RECRUITER_TABLE, readBody(req), config)
    return json(res, 200, { ok: true, row: result.row, message: result.message, emailSent: result.emailSent, emailError: result.emailError })
  } catch (error) {
